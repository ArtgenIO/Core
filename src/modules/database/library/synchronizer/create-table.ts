...
export const doCreateTable = async (schema: ISchema, connection: Knex) => {
  const query = connection.schema.createTable(schema.tableName, table => {
  ...
  console.log('Doing', query.toString());
  await query;
};
